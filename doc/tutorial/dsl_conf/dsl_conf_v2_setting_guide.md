...
[`中文`](dsl_conf_v2_setting_guide_zh.rst)

To make the modeling task more flexible, currently, FATE uses its own domain-specific language(DSL) to describe modeling task. With usage of this DSL, modeling components such as data-io,
...
of [`V1`](dsl_conf_v1_setting_guide.md)

## DSL Configure File

### 1. Overview

`DSL Configure File(DSL)` is a json format file used to defines

- job's components
- data flows
- model flows

There is only one field `components` in `DSL`,
```json
{
  "components" : {
    ...
...
```
and then each component should be defined on the second level field: 
...
where `<component name>` must be json-compatible string and `<Module name>` should be one of
[Algorithms, fix me](...)


### 3. Define data flow and model flow
...
-  **definition:** There are two types of input, data and model.
    ...
    - 4.1 Data Input
        ...
        -  **definition:**  Data input from previous modules.
          
            there are four possible data_input type:
